...
/***/ function(module, exports) {
	...
	var linesIntersect = function linesIntersect(lineA, lineB) {
	...
	var Bar = function Bar(x, y, radius, rotation, thickness, ctx) {
	...
	var Marble = function Marble(x, y, angle, bars, ctx) {
		var _x = x,
		    _y = y,
		    x1 = x,
		    y1 = y,
		...
		function gravAcceleration() {
			return ang > rad90 ? (rad180 - ang) * 0.008 : ang * 0.008;
		}

		function downwardCollision() {
			for (var i = 0; i < bars.length; i++) {
				var collides = linesIntersect(bars[i].getLineVector(), [_x, _y, _x + Math.cos(rad90) * acc, _y + Math.sin(rad90) * acc]);
				if (collides) {
					return {
						collides: true,
	...
	var bars = [new Bar(255, 250, 250, 25, 3, ctx), new Bar(350, 500, 250, 350 - 180, 3, ctx)];

	var marbles = [new Marble(260, 10, 20 * (Math.PI / 180), bars, ctx), new Marble(250, 10, 140 * (Math.PI / 180), bars, ctx), new Marble(265, 10, 22 * (Math.PI / 180), bars, ctx), new Marble(240, 10, 138 * (Math.PI / 180), bars, ctx)];
